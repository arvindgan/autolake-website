...
const NetworkBackground = ({ mounted }: { mounted: boolean }) => {
...
const TestimonialCarousel = ({ testimonials, mounted }: { 
  ...
  return (
    <div className="relative h-64 overflow-hidden">
      {testimonials.map((testimonial: { quote: string; author: string; company: string }, index: number) => (
        <motion.div
          key={index}
...
const IndustryCard = ({ industry, onClick }: { industry: Industry; onClick: () => void }) => {
  ...
  const getIconAnimation = (industryId: string) => {
    switch (industryId) {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col h-full relative z-10">
        ...
        >
          <motion.div className={`p-3 rounded-full ${getColorClass(industry.color)}`}>
            {industry.icon}
          </motion.div>
...
const IndustryCardOld = ({ industry, onClick }: { industry: Industry; onClick: () => void }) => {
  return (
    <motion.div
      className={`p-6 rounded-lg border cursor-pointer transition-all duration-300 overflow-hidden relative ${getColorClass(industry.color).replace("text-", "border-").replace("400", "500/30")}`}
...
const IndustryDetail = ({ industry, onBack }: { industry: Industry; onBack: () => void }) => {
  return (
    <motion.div
          initial={{ opacity: 0, y: 20 }}
